...
<main class="container-fluid">
  <section class="py-5  container">
    <div class="row py-lg-5">
      <div class="col-6">
        <ul class="nav nav-tabs" id="myTab" role="tablist"> @foreach($courses as $key => $item) <li class="nav-item" role="presentation">
          <button class="nav-link {{ $key == 0 ? 'active' : '' }}" id="courseId" onclick="courseId({{ $item->id }})"  data-bs-toggle="tab" data-id="{{ $item->id }}" data-bs-target="#home{{ $item->id }}" type="button" role="tab" aria-controls="home" aria-selected="true">{{ $item->levelcourse->name }}</button>
          </li> @endforeach </ul>
        <div class="tab-content" id="myTabContent"> @foreach($courses as $key=> $course) <div class="tab-pane fade show {{ $key == 0 ? 'active' : '' }}" id="home{{ $course->id }}" role="tabpanel">
            <table class="table">
              <thead>
                <tr>
              ...
              <tbody> @php $assign_courses = App\Models\AssignCourse::where('course_id',$course->id )->with('course')->get(); @endphp @foreach($assign_courses as $item) <tr>
                  <td scope="row">{{ $item->date }}</td>
                  <td scope="row">{{ $item->course->category->name }} {{ ($item->course->levelcourse->name) }}</td>
                  {{-- <td scope="row">{{ $item->course->citycourse->city->name }}</td> --}} <td scope="row">{{ $item->course->status }}</td>
                  <td scope="row">
      ...
      <div class="col-lg-3 col-md-6 col-sm-12">
        <div class="card">
          ...
          <div class="card-body">
            <div class="row" id="myHtmlCode"></div>
          </div>
        </div>
      </div>
      <div class="col-lg-3 col-md-6 col-sm-12">
        <div class="macro-course-structure course-blue-box large">
          <div class="title accordion">
            <h2>
              <span class=" text-white"> Power BI Course Outlines</span> 
            </h2>
          </div>
                ...
                <div class="container-flex">
                  <div class="row">
                    <div class="col">
                      ...
                      <div class="download">
                        <a href="https://portal.nexacu.com.au/api/Public/GetCoursePdf/10" target="_blank">download course pdf</a>
                      </div>
                      <div>
                        <span>$385</span>
                      </div>
                    ...
                    <div class="col d-flex align-items-center justify-content-end pb-3">
                      <a href="/microsoft-power-bi-training-courses/power-bi-beginner/" class="view-course">View <span class="courses">Course</span>
                      </a>
                    </div>
                  </div>
                </div>
